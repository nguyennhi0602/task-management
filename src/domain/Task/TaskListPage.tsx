...
export const TaskListPage = () => {
  ...
  const [isOpenDeleteTaskConfirmModal, setOpenDeleteTaskConfirmModal] = useState(false)
  ...
  const onDragEnd = (result: any) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const destinationTasks = getTasksByCategory(Number(destination.droppableId))
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = moment()
      const dueTasks = mockData.filter((task) => task.deadlineAt === now.format("YYYY-MM-DD"))
  ...
  return (
    <RequireAuth>
      <MainBody>
          ...
          <Container>
            {Object.values(categories).map((category) => (
              <Droppable key={category.id} droppableId={`${category.id}`}>
                {(provided) => (
                  <TaskColumn ref={provided.innerRef} {...provided.droppableProps}>
                    <h3>{category.name}</h3>
                    <TaskWrapper>
        ...
        <TaskDetailDialog open={isOpenTaskDetails} taskDetails={selectedTask} onClose={handleClose}></TaskDetailDialog>
        <CreateTaskModal
